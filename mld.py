...
st.markdown("<h1 style='text-align: center;'>Mixed Layer Depth Analysis from CTD Profiles 📏🌊</h1>", unsafe_allow_html=True)

# Access the shared data from session state
data = st.session_state.get('data')

if data is None:
    st.error("Data not found in session state. Please load data in the main application.")
else:
    ...
    selected_grids = st.sidebar.multiselect('Select Grid(s)', grid_options, default=[grid_options[1]])

    # Season and Year filters
    selected_season = st.sidebar.multiselect('Select Season(s)', data['season'].unique(), default=data['season'].unique()[0])
    selected_year = st.sidebar.multiselect('Select Year(s)', data.datetime.dt.year.dropna().unique(), default=data.datetime.dt.year.dropna().unique()[0])
    ...
    if not filtered_data.empty:
        ...
        st.warning("No data selected. Please select at least one grid to visualize the data.")
